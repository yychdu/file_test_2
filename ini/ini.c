...
static int ValueGet(int ch, FILE* fp, int size, char *value, int brackets_flag, int *result, int *check_fin) 
{
    ...
    *check_fin = 0;
    ...
    {   
        ...
        *result = j;
        if (brackets_flag && pos == ']')
            *result = -3; 
        else if (pos_len >= size)
                ; //save ' '
        else if (j != 0)
        {
            //rm ' '
            i = j - 1;
            while ((unsigned)value[i] <= ' ')
                i--;
            *result = i + 1;
        }    
...
int main()
{
    ...
    printf("input key:\n");
    scanf("%s", key);
    int k=IniReadStr(filename, section, key, value, 10);
    if (k>=0)
