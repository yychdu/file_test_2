...
int BracketsCheck(int ch, FILE* fp)             //ch如果是 ']'会被跳掉
{
    long k = ftell(fp);
    while (ch != '\n' && ch != EOF)                 //又问题
    {
    ...
    fseek(fp, k - ftell(fp), SEEK_CUR);
    return 0;
}

int ValueGet(int ch, FILE* fp, int size, char *value, int brackets_flag, int *result, int *line_status)
{
    int j;
    while (_IsBlank(ch))
    ...
    if (ch == '=')    //后面是等号 找到了
    {
        ch = CleanBlank(fp);  //排除‘=’后空格   如果是换行符 j=0 会出问题
        for (j = 0; j < size && ch != '\n' && ch != EOF; j++)     //value和size要检查
        {                                                   
            value[j] = ch;
            ch = fgetc(fp); 
        }                                                 
        *result = j;
        if (value[j - 1] == ']' && brackets_flag == 1)      //有问题
            *result = -3;
        line_status = 0;
    }
    else    //不是等号 只有一部分   下一行
...
{  
    int  key_size = 0,line_status = 0, brackets_flag = 0, check_fin = 0;   
    int result = -2;
    ...
    else
    {
        ...
        {          
            ...
            else if (line_status == 1)              //状态3：查找KEY
            {
                ...
                if (key[key_size] == '\0')
                {
                    ch = ValueGet(ch, fp, size, value, brackets_flag, &result, &line_status);
                    if (line_status == 0)
                        break;
                }
                else
