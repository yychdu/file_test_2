...
int ValueGet(int ch, FILE* fp, int size, char *value, int brackets_flag, int *result, int *check_fin) 
{
    int j = 0, i = 0, step = 0,cnt = 0; 
    *check_fin = 0;
    while (_IsBlank(ch))
    ...
    else if (brackets_flag && BracketsCheck(ch, fp))
        *check_fin = 1;
    else 
        ch = NextLine(ch, fp);
    return ch = fgetc(fp);
